...
def initialize_from_env_for_eval():
  if "GPU" in os.environ:
    set_gpus(int(os.environ["GPU"]))
  else:
    set_gpus()
  ...
  # get the configuration from the logs
  config = pyhocon.ConfigFactory.parse_file("logs/%s/experiments.conf"%(name))[name]
  config["log_dir"] = mkdirs(os.path.join(config["log_root"], name))
...
def ffnn(inputs, num_hidden_layers, hidden_size, output_size, dropout, output_weights_initializer=None):
  ...
  #outputs = tf.nn.softmax(tf.nn.xw_plus_b(current_inputs, output_weights, output_bias))
  
  if len(inputs.get_shape()) == 3:
    outputs = tf.reshape(outputs, [batch_size, seqlen, output_size])
  return outputs

def ffnn_softmax_output(inputs, num_hidden_layers, hidden_size, output_size, dropout, output_weights_initializer=None):
  if len(inputs.get_shape()) > 3:
    raise ValueError("FFNN with rank {} not supported".format(len(inputs.get_shape())))
    ...
    if len(path) > 0:
      ...
      with open(path) as f:
        for i, line in enumerate(f.readlines()):
          if "w2v" in path and i == 0:  ### the first line in w2v.txt is the vocabulary_size and word dimention
            continue
